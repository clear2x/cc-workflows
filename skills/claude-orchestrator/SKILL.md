...
脚本路径（Hermes 全局）：`/Users/clear2x/.hermes/skills/claude-orchestrator/claude_orchestrator.py`
脚本路径（Claude Code 用户级）：`~/.claude/skills/claude-orchestrator/claude_orchestrator.py`
...
```bash
python3 ~/.hermes/skills/claude-orchestrator/claude_orchestrator.py run "任务描述" --agent Explore
python3 ~/.hermes/skills/claude-orchestrator/claude_orchestrator.py run "任务描述" --agent Plan --model step-3.7-flash
```
...
```bash
python3 ~/.hermes/skills/claude-orchestrator/claude_orchestrator.py pipeline \
...
```bash
# 每行一步，自动分段执行
python3 ~/.hermes/skills/claude-orchestrator/claude_orchestrator.py loop "Step 1: list files
Step 2: read calculator.py
Step 3: read logger.py
Step 4: report findings"

# 指定最多 50 段
python3 ~/.hermes/skills/claude-orchestrator/claude_orchestrator.py loop "Step 1: ..." --max-steps 50

# 指定 agent
python3 ~/.hermes/skills/claude-orchestrator/claude_orchestrator.py loop "Step 1: ..." --max-steps 100 --agent Explore
```
...
```bash
python3 ~/.hermes/skills/claude-orchestrator/claude_orchestrator.py sessions
```
...
> 用 claude-orchestrator loop 重构 src/，最多 50 步
...
```
~/.claude/skills/claude-orchestrator/
├── SKILL.md          ← 必须存在，否则 Claude Code 不会加载此 skill
└── claude_orchestrator.py
```

如果 `~/.claude/skills/claude-orchestrator/` 下没有 `SKILL.md`，需要手动复制 Hermes 全局版本的 `SKILL.md` 过去。

## 安装位置

| 作用域 | 路径 | 说明 |
|--------|------|------|
| Hermes 全局 | `/Users/clear2x/.hermes/skills/claude-orchestrator/claude_orchestrator.py` | Hermes Agent 使用 |
| Claude Code 用户级 | `~/.claude/skills/claude-orchestrator/claude_orchestrator.py` + `SKILL.md` | Claude Code 自动加载 |
| 项目级 | `.claude/skills/claude-orchestrator/SKILL.md`（文档）+ 脚本软链接 | 项目共享 |
...
Claude 会自动加载 Superpowers 的 `writing-plans`、`subagent-driven-development`、`test-driven-development` 等 skill，同时 `claude-orchestrator` 会被触发用于长任务循环。

### 推荐组合

| Superpowers skill | claude-orchestrator 模式 | 用途 |
|-------------------|--------------------------|------|
...
`claude-orchestrator` 已内置 Superpowers 工作流约束自动注入。loop 模式下，每段会根据关键词自动注入对应 skill 的规范（TDD、writing-plans、subagent-driven-development 等）。

详见：`references/superpowers-integration.md`

## 注意事项

- 脚本路径：`~/.hermes/skills/claude-orchestrator/claude_orchestrator.py`
