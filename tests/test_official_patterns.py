...
class TestFanout:
    def test_subtasks_execute_and_synthesize(self, tmp_path, monkeypatch):
        ...
        with patch.object(orch, "run_claude", side_effect=fake_run_claude), \
            ...
            for t in subtasks:
                fut = Future()
                # Call the internal _fanout_subtask directly
                res = orch._fanout_subtask(t, str(orch.WORKTREE_BASE))
                fut.set_result(res)
                futures[fut] = t
...
class TestGenfilter:
    def test_generates_and_filters(self, tmp_path, monkeypatch):
        ...
        with patch.object(orch, "run_claude", side_effect=fake_run), \
             patch.object(orch, "_fanout_subtask") as mock_subtask:
            mock_subtask.side_effect = lambda cfg, base: {
                "name": cfg["name"],
                "error": False,
        ...
        # _fanout_subtask should have been called 3 times (count=3)
...
class TestTournament:
    ...
    def test_insufficient_contestants_exits(self, tmp_path, monkeypatch):
        ...
        with patch.object(orch, "run_claude") as rc, \
            ...
            with patch.object(orch, "_fanout_subtask",
